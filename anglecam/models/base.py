...
class DINOv2_AngleCam(nn.Module):
    def __init__(self, cfg: DictConfig):
        ...
        # Load the backbone
        self.backbone = torch.hub.load('facebookresearch/dinov2', self.cfg.model.name, pretrained=True)
    ...
    def _create_head(self) -> nn.Module:
        return nn.Sequential(
            nn.Linear(self.cfg.model.backbone.hidden_dim, self.cfg.model.head.hidden_dims[0]),
            nn.Dropout(self.cfg.model.head.dropout),
            nn.GELU(),
            nn.Linear(self.cfg.model.head.hidden_dims[0], self.cfg.model.output.num_bins),
