...
class AngleCamPredictor:
    ...
    def _predict_batch(
            ...
            for batch_idx, (batch_tensors, batch_paths) in enumerate(dataloader):
                ...
                try:
                    ...
                    if "out of memory" in str(e):
                        ...
                        raise RuntimeError(
                            f"GPU out of memory. Try reducing batch_size from {batch_size}"
                        )
                    raise
                ...
                batch_results = []
                for i, (probabilities, image_path) in enumerate(
                    zip(batch_probabilities, batch_paths)
                ):
                ...
                current_rate = (
                    processed_images / elapsed_time if elapsed_time > 0 else 0
                )
        ...
        self.logger.info(
            f"Processed {total_images} images in {total_time:.2f}s ({final_rate:.1f} img/s)"
        )
